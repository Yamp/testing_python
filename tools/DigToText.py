...
def num2text(num, main_units=((u'', u'', u''), 'm'), asDigits=False):
    """
    ...
    if asDigits:
        rest = abs(num)
        name = []
        plural, nme = thousand(rest % 1000, _orders[0][1])
        name.append(_orders[0][0][plural])
        name.append(str(num))
    else:
        if num == 0:
...
def decimal2textRub(value: float, isCopAsText = False):
    int_units = ((u'рубль', u'рубля', u'рублей'), 'm')
    exp_units = ((u'копейка', u'копейки', u'копеек'), 'f')
    return decimal2text(decimal.Decimal(str(value)), int_units=int_units, exp_units=exp_units, isCopAsText = isCopAsText)


def decimal2text(value, places=2,
                 int_units=(('', '', ''), 'm'),
                 exp_units=(('', '', ''), 'm'), isCopAsText = False):
    ...
    return u'{} {}'.format(
        ...
        num2text(int(exp), exp_units, not isCopAsText))
